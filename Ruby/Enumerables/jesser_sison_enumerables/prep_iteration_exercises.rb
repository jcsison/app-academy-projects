...
class Array
  ...
  def bubble_sort(&prc)
    array = self.map(&:itself)
    prc ||= proc { |a, b| a <=> b }
    nil while (0..array.length - 2).any? do |i|
...
def substrings(string)
  string.length.times.each_with_object([]) do |i, substrings|
    (i..string.length - 1).each { |j| substrings << string[i..j] }
  end
end

def subwords(word, dictionary)
  substrings(word).uniq.select { |substring| dictionary.include?(substring) }
end
...
class Array
  def my_map(&prc)
    array = []
    self.my_each { |item| array << prc.call(item) }
    array
  end
  ...
  def my_inject(&blk)
    acc = self[0]
    (1..self.length - 1).to_a.my_each { |i| acc = blk.call(acc) }
    acc
  end
end
